...
public class Racing {
    ...
    protected void updateGUI() {																
    	...
    	//Отобразить на кнопке RaceState текущее состояние игры
        ...
        for (Missile missile : missiles) {
            for (Player player : racers) {
                ...
                if (collide(missile, player.getCar())) {
                    missile.collideWith(player.getCar()); //обработать попадание снаряда
    ...
    private void finishRace() {
        ...
        // Добавить всех игроков в список закончивших гонку
        for (Player player : racers) {
            finished.add(player);
        }
        for (Player player : finished) {
